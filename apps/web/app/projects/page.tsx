    ...
    <main className="page">
      <section className="hero">
        ...
        <Surface className="onboarding-card" as="section">
          <div className="onboarding-card-head">
            <Badge tone="outline">First-run path</Badge>
            <strong>Create a project, choose an artifact type, then jump straight into Studio.</strong>
          </div>
          <div className="onboarding-steps">
            <div className="onboarding-step">
              <span>1</span>
              <p>Name the workspace you want to explore.</p>
            </div>
            <div className="onboarding-step">
              <span>2</span>
              <p>Pick website, prototype, or slides from that project card.</p>
            </div>
            <div className="onboarding-step">
              <span>3</span>
              <p>Open Studio, shape the first pass, then save a snapshot.</p>
            </div>
          </div>
        </Surface>
      </section>
      ...
      <div className="projects-grid">
        {projectsWithArtifacts.length === 0 ? (
          <Surface className="project-card onboarding-card" as="article">
            <div>
            ...
            <div className="starter-grid">
              <Surface className="kv" as="section">
                ...
                Create one project and start with a website artifact if you want the
                fastest full loop from scene to export.
              </Surface>
              <Surface className="kv" as="section">
                <span>Signed-in mode</span>
                Auth keeps projects scoped to your account across restarts and Docker
                rebuilds.
              </Surface>
              <Surface className="kv" as="section">
                <span>Local mode</span>
                You can still use the lightweight browser-only flow without signing in.
              </Surface>
            </div>
          </Surface>
        ) : null}
        ...
        {projectsWithArtifacts.map(({ project, artifacts }) => (
          <Surface key={project.id} className="project-card" as="article">
            ...
            {artifacts.length === 0 ? (
              <Surface className="kv" as="section">
                <span>Choose the first artifact</span>
                Start with `website` for the quickest end-to-end loop, `prototype` for
                flow structure, or `slides` for narrative pacing.
              </Surface>
            ) : null}
            <div className="hero-actions">
